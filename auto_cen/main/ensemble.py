...
from auto_cen.constants import MULTILABEL, ACCURACY, DOUBLEFAULT, SILHOUETTE
...
class EnsembleLearner:
    ...
    def _optimize_classifiers(self, n_processes):
        ...
        cs_models = create_cs(self.classifiers,
                              data_characteristics=self.data_characteristics,
                              has_cat=len(self.data.cat_mask) > 0, default_value="RF")
    ...
    def _compute_best_ensemble(self, selector: Selector, n_processes: int) -> (
        ...
        elif len(self.ensemble_size) == 2:
            size_range = [self.ensemble_size[1]]
        else:
            size_range = self.ensemble_size
    ...
    def _compute_ensemble(self, selector: Selector, k: int, n_processes: int) -> (
        ...
        sel = None
        if isinstance(self.ensemble_size, Tuple) and len(self.ensemble_size) == 2:
            k = self.ensemble_size
            sel = selector

        self.combiner_simple, self.combiner_hp = get_combiner(self.specification)

        cs_combiner = create_cs(self.combiner_hp + self.combiner_simple,
                                ens_size=k, default_value="MAMV")
        ...
        try:
            ...
            logger.info("Fusion Training Failed")
            selected_combiner = RunInfo("Failed",{"ENS:size":-1}, 000, {"cost": np.inf},[],[],-1, -1)
            models = []
